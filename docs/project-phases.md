...
| Pipeline orchestration | `Bus::chain()` for sequential jobs (Search → AI → Send); `dispatch()` for typing (fire-and-forget) |
| Pipeline state carrier | `catalog_requests` table — jobs share state by reading/writing this record using its ID |
| Pipeline status | `catalog_pipeline_statuses` lookup table (seeded values) — follows project convention |
| WhatsApp jobs | Stub infrastructure only — job class + method signature exist, actual API call is a TODO |
| Entry point | `POST /api/catalog/pipeline` (Sanctum auth); adaptable to a webhook receiver |
...
```
POST /api/catalog/pipeline
  { question, contact_id }
         │
         ▼
  CatalogPipelineService::dispatch()
         │
         ├─── dispatch(WhatsAppTypingJob)          [fire-and-forget, parallel]
         │         └── calls WhatsApp typing API   [STUB — structure only]
         │
         └─── Bus::chain([
                CatalogSearchJob($requestId)
                  └── CatalogSearchService → stores search_results on CatalogRequest
                GenerateCatalogAnswerJob($requestId)
                  └── CatalogAnswerService (from pre-loaded results) → stores ai_answer on CatalogRequest
                SendWhatsAppMessageJob($requestId)
                  └── reads ai_answer + contact_id → calls WhatsApp send API  [STUB — structure only]
              ])->dispatch()
         │
         ▼
...
Every job in the chain receives only the `$catalogRequestId`. Each job loads the `CatalogRequest` record, performs its work, updates the record, and the chain continues.
